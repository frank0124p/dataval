...
mindmap
  root((config 知識庫))
    Common
      實體：sample_child、sample_parent
      SSOT 權威：customer、product
      規則：閘門 23、顧問 3
    BLM
      實體：blm_detail、blm_master
      規則：閘門 1
    CRM
      實體：dim_customer、order_items、orders
      流程：order_to_revenue
      產品：om、pi
      規則：閘門 1
    FCM
      實體：fcm_detail、fcm_master
      規則：閘門 1、顧問 1
    PLM
      實體：bom、bom_line、part_master
      流程：eco_to_bom
      規則：閘門 2、顧問 4
    SCM
      實體：purchase_order、purchase_order_line、supplier
      流程：procure_to_pay
      規則：閘門 3、顧問 1
```
