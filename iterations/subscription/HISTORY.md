...
- 代填待驗證：`CONCEPT.SUBJECT@billing_event`、`CONCEPT.SUBJECT@subscription`、`CONCEPT.SUBJECT@subscription（方案變更）`、`NAME.SEMANTIC@subscription.subscription_id`、`SKILL.best_practice_semantic@billing_event`、`SKILL.best_practice_semantic@billing_event（遲到資料與封帳）`、`SKILL.best_practice_semantic@subscription`、`SKILL.naming_semantic@billing_event.event_id`、`SKILL.naming_semantic@subscription.customer_id / dim_customer.customer_id / billing_event.customer_id`、`SKILL.naming_semantic@subscription.started_at / subscription.created_at`、`SKILL.ssot_semantic@billing_event.amount（幣別）`、`SKILL.ssot_semantic@dim_customer.customer_tier（過渡期權威）`、`SKILL.ssot_semantic@dim_customer（本主體） vs CRM 客戶主檔`、`SKILL.ssot_semantic@subscription.customer_email`
- input 變更（vs 第 1 輪）：answers.yaml
- 發現變化（vs 第 1 輪）：新增 9、解決 16、狀態變化 0（明細：round_2.delta.md）
