...
> 這份是**細節**：架構、規則系統、正式區治理、lineage、DataHub 整合、
> 迭代問答的內部機制與設計理由。
>
> 想知道**怎麼用**看 [`README.md`](README.md)；第一次接觸看
> [`QUICKSTART.md`](QUICKSTART.md)；輸入格式看 [`input/README.md`](input/README.md)；
> 寫規則看 [`SKILL_AUTHORING.md`](SKILL_AUTHORING.md)。

一句話：**閘門用確定性規則，LLM 只進顧問區。** 合規判定永遠零 LLM，
同一輸入永遠同一結果。
...
| 閘門 | **預檢**（試跑草稿 DDL，設計參考、非正式判定）；不套用 `BUSINESS_KEY.METADATA`——context 的 business_keys 是**來源表**的鍵，設計產出的是整合後的新表 | 正式判定，全部規則都算 |
...
| 層 | 誰做 | 判準 | 產出 |
|---|---|---|---|
| 確定性 | 工具（零 LLM） | `relations.yaml`／設計稿的三段式端點 | 閘門 `PRODUCTION.REUSE` 警告（不擋）＋問答題 |
| 語意 | agent 顧問區 | 要承載的事實是否已由已核准主體承載（**欄名不同也算**） | `SKILL.production_reuse_semantic` 建議（一律 info）＋代填答案進 `answers.yaml` |

語意層的素材由工具備好：正式區資產的表／欄位／用途，加上**同名欄位候選**
（排除稽核欄雜訊、join key 優先、已引用的不再列）。這些只是線索——
同名不一定同義，該不該複用由語意判讀決定。

**沒複用就顯性提醒**（確定性層的判定只認確定性證據）：
