...
**省掉不必要的 LLM——照 run.py 的指示做就好**（②是整條流程最貴的一步，
一個主題要讀 ~24K 字元、寫 ~8K 字元）：

- run.py 印 `♻️ 顧問區可沿用` ⇒ 那些主題的 prompt 與上輪**位元組相同**，
  上次的建議仍然成立。**不要重讀 prompt、不要重產 result**，直接跳到 ③。
- run.py 印 `待補：<主題>（原因）` ⇒ 那個主題真的變了（原因會寫明是 prompt
  變了還是 result 被改過），要重補。
- **多個主題一次平行做完**——它們彼此獨立，沒有任何順序相依，
  一個一個等只是白白拉長時間。

新 DDL 使用一 subject 一資料夾（DDL 可多檔拆放＝一組表：主檔 `<名稱>.sql`
之外，同資料夾其餘 `*.sql` 一併載入，`derivation.sql` 除外）：三件必備
`input/<名稱>/<名稱>.sql`、
