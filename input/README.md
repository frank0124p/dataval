...
報告的顧問區會產出「給設計者思考的提問」。回答後重跑即進入
**迭代收斂迴圈**（收斂條件：無待答問題＋閘門合規；上限 5 輪；
狀態見報告的「迭代收斂」區塊）。每輪 `merge_advisory.py` 會產出
`reports/<名>.answers_draft.yaml` 草稿（含 agent 建議答案）。

**標準回答路徑**：agent（opencode／Claude Code）在 command line 對話中
逐題向你提問（附建議答案），你直接在對話裡回答（接受／修改／擱置），
agent 依你的回覆代筆回寫本檔——每一筆都必須出自你的明確回覆，
agent 不得自行編造或未經回覆採納建議。你也可以直接手動編輯本檔，效果相同。
