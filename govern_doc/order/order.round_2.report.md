# 資料設計驗證報告 — 第 2 輪迭代
_產生時間 2026-09-08T14:17:33.741897Z_<br>
**🔁 第 2／5 輪迭代報告**<br>
**判定：✅ 合規**（會擋項目 0）<br>
通過 37 · 警告 6 · 失敗 0 · 略過 9 · 提示 17<br>
閘門區 52 項 · 顧問區 17 項<br>
...
- ℹ️ 未實檢／略過：`DATAHUB.ACCESS_GRANT`、`DATAHUB.COLUMN_DESC`、`DATAHUB.LINEAGE`、`DATAHUB.OWNER`、`DATAHUB.QUALITY_CHECK`、`DATAHUB.TABLE_DESC`、`DATAHUB.TAG`、`SKILL.crm_baseline`、`SKILL.structural_fk_resolves`
...
### 🔄 與第 1 輪相比的發現變化
- 新增 19、解決 16、狀態變化 0（明細：iterations/<名>/round_2.delta.md；該輪完整報告：round_2.report.md）
...
| 表 | 平台位置（URN） | 位置來源 |
|---|---|---|
| `order_items` | `urn:li:dataset:(urn:li:dataPlatform:clickhouse,order_items,PROD)` | 依表名與 config/_engine/datahub.yaml 推導 |
| `orders` | `urn:li:dataset:(urn:li:dataPlatform:clickhouse,orders,PROD)` | 依表名與 config/_engine/datahub.yaml 推導 |
